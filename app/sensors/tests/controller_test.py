...
from apsharedp.cassandra_client import CassandraClient
import time
...
def clear_dbs():
    ...
    ts = Timescale()
    ts.execute("DROP TABLE IF EXISTS sensor_data CASCADE")
    ts.execute("commit")
    # ts.execute("DELETE FROM sensor_data")
    ts.close()
    from app.main import apply_migrations  
    apply_migrations()
    ...
    while True:
        try:
            ...
            time.sleep(5)
    from app.main import create_cassandra
    create_cassandra()
...
def test_post_sensor_data_temperatura_2():
    response = client.post("/sensors/1/data", json={"temperature": 4.0, "humidity": 1.0, "battery_level": 1.0, "last_seen": "2020-01-01T00:00:02.000Z"})
    assert response.status_code == 200
...
def test_post_sensor_data_temperatura_4():
    response = client.post("/sensors/4/data", json={"temperature": 17.0, "humidity": 1.0, "battery_level": 1.0, "last_seen": "2020-01-02T00:00:02.000Z"})
    assert response.status_code == 200
